    ...
    $.fn.taxelect = function(options) {
        ...
        this.each(function() {
            ...
            $(this).children().each(function() {
                ...
                arr.selected = $(this).attr('selected') ? true : false;
            ...
            default_name = selectOptions[0].text;
            default_id = 0;
            for (var i = 0; i < selectOptions.length; i++) {
                var icon = selectOptions[i].hasIcon ? '<span id="' + selectFormId + '-' + selectName + '-option-' + i + '-span' + '" class="taxelect-icon"></span>' : '';
                if (selectOptions[i].selected) {
                    default_name = selectOptions[i].text;
                    default_id = i;
                }
                tmp += '<li class="taxelect-option" data-option="' + i + '">' + selectOptions[i].text + icon + '</li>';
            }
            // add needed elements after select
            if (settings.defaultOption == 'auto') {
                $(this).after('<div class="taxelect-wrapper">' + '<ul class="taxelect-ul" data-form-id="' + selectFormId + '" data-name="' + selectName + '">' + '<li class="taxelected" data-default-option="' + default_id + '">' + default_name + '</li>' + tmp + '</ul></div>');
            } else {
                $(this).after('<div class="taxelect-wrapper">' + '<ul class="taxelect-ul" data-form-id="' + selectFormId + '" data-name="' + selectName + '">' + '<li class="taxelected" data-default-option="' + settings.defaultOption + '">' + settings.defaultString + '</li>' + tmp + '</ul></div>');
            }
            ...
            selectWrapper.click(function() {
                ...
                if (selectWrapper.hasClass('opened')) {
                    ...
                    }, settings.closingSpeed, function() {
                    ...
                    if (other.length) {
                        selectWrapper.addClass('opened').promise().done(function() {
                            selectWrapper.animate({
                                height: settings.openedHeight
                            }, settings.openingSpeed);
                        });
                        other.animate({
                            height: settings.closedHeight
                        }, settings.closeOtherSpeed, function() {
